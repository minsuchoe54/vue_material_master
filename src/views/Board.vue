    ...
    updateParams(newProps) {
      ...
      async getFromServer(params){
        try {
          ...
          const response =  await axios.get('https://op.cc-oo.club/api/infos?pagination[page]='+this.serverParams.page+'&pagination[pageSize]='+this.serverParams.perPage+'&[fields][0]=name&[fields][1]=storetype&[fields][2]=phone&&sort[0]='+this.serverParams.sort[0].field+'%3A'+this.serverParams.sort[0].type+keyword,{headers: {Authorization: "Bearer "+this.$store.state.token}})
      ...
      loadItems() {
            this.getFromServer(this.serverParams).then(response => {
              ...
              response.data.data.forEach(element => {
                this.rows.push({id:element.id,name:element.attributes.name,storetype:element.attributes.storetype,phone:element.attributes.phone})
              })
